...
const defaultSettingsOptions = [
  ...
  { name: "Subscription transaction", href: "/pruser/subscription-traction" },
...
export default function SettingsPage() {
    ...
    if (email) {
      fetch(`/api/users?email=${email}`)
        ...
        .then((data) => {
          ...
          setIsSubscribed(data.subscribe) // Set subscription status
        })
  ...
  const handleSubscribe = async (plan) => {
    setLoading(true)
    const amount = plan === "pro" ? 9900 : 9900
  ...
  return (
    <>
      ...
      >
        <motion.h1 initial={{ y: -20 }} animate={{ y: 0 }} className="text-4xl font-bold mb-8 top-1 text-white">
          ⚙️ Settings
        </motion.h1>
              ...
              {option.name === "Subscription" ? (
                ...
                <MotionLink
                  ...
                  className={`block p-4 rounded-lg shadow-md text-center text-lg font-medium transition-colors ${
                    pathname === option.href ? "bg-blue-600 text-white" : "bg-white/10 text-white hover:bg-white/20"
                  }`}
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.98 }}
                >
